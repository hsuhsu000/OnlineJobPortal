<?php
	session_start();
	include "dbconnect.php";

	// if($_SESSION['user_array']['role'] != 'CompanyRepresentative'){
	// 		header('location: jobseeker-dashboard.php');
	// 	}


?>


<!DOCTYPE html>
<html>
<head>
	<title></title>
	<link rel="stylesheet" type="text/css" href="stylesheet.css">
	<link rel="stylesheet" type="text/css" href="css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" href="fontawesome/css/all.min.css">
	<script src="js/jquery.min.js"></script>
	<script src="js/bootstrap.min.js"></script>
	<script src="js/bootstrap.bundle.min.js"></script>

	
</head>
<body>


	<!------------ Navbar -->
	<nav class="navbar navbar-expand-lg navbar-light">
		  <a class="navbar-brand" href="#">Online Job Portal</a>
		  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavAltMarkup" aria-controls="navbarNavAltMarkup" aria-expanded="false" aria-label="Toggle navigation">
		    <span class="navbar-toggler-icon"></span>
		  </button>
		  <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
		    <div class="navbar-nav">
		      <a class="nav-link active" href="jobseeker-dashboard.php">Home <span class="sr-only">(current)</span></a>
		      <a class="nav-link" href="findJobs.php">Find Jobs</a>
		      <a class="nav-link" href="view_applied_jobs(jobseeker).php">
		      	View Your Jobs

		      	<i class="fa fa-user" style="color: black; font-size:20px;"></i>
		      	
		      </a>
		    </div>
		  </div>

		  <a href="logout.php"><button class="btn btn-secondary btn-lg">Logout</button></a>
	</nav>

	<!--------------------------- Body -->
	<div class="container mt-5 mb-5">
		<div class="row">
			<!----------------- VIEW JOBS ---------------->
			<div class="col-lg-12 col-md-12 col-sm-12">
				<?php 
				$jid_to_view = $_GET['jid_to_view'];
				$query = "SELECT * FROM jobs WHERE jid = $jid_to_view";
				$result = mysqli_query($dbconnection,$query);
				if($result){

					$jobarr = mysqli_fetch_assoc($result);
					$_SESSION['jobarr'] = $jobarr;
				}
				else{
					die('Error: '.mysqli_error($dbconnection));
				}
			?>
			<div class="card">
				<div class="card-header">
					<div class="card-title text-center"><h3><?php echo $jobarr['jobTitle']; ?></h3></div>
				</div>
				<div class="card-body">
					<table class="table table-bordered table-condensed ">
						<tr>

							<td>Comapany Logo</td>
							<td>
								<img src="<?php echo 'images/'.$jobarr['companyLogo']?>" width="100px" height="100px">
							</td>
						</tr>
						<tr>
							<td>Company Name</td>								
							<td><?php echo $jobarr['companyName'];?></td>	
						</tr>
						<tr>
							<td>Job Type</td>
							<td><?php echo $jobarr['jobType']; ?></td>
						</tr>
						<tr>
							<td>Job Category</td>
							<td><?php echo $jobarr['category']; ?></td>
						</tr>
						<tr>
							<td>Job Title</td>
							<td><?php echo $jobarr['jobTitle']; ?></td>
						</tr>
						<tr>
							<td>Job Description</td>
							<td><?php echo $jobarr['jobDescription']; ?></td>
						</tr>
						<tr>
							<td>Skill Required</td>
							<td><?php echo $jobarr['skillRequired']; ?></td>
						</tr>
						<tr>
							<td>Job Location</td>
							<td><?php echo $jobarr['jobLocation']; ?></td>
						</tr>
						<tr>
							<td>Salary</td>
							<td><?php echo $jobarr['salary']; ?></td>
						</tr>
						<tr>
							<td></td>
							<td><a href="job_submission_form.php?jid_to_view_application=<?php echo $jobarr['jid']; ?>" class="btn btn-secondary btn-lg">Apply Job</a>
							<a href="jobseeker-dashboard.php" class="btn btn-secondary btn-lg">Back</a>
							</td>
						</tr>
					</table>					
				</div>
			</div>
			</div>						
		</div>
	</div>
	

	<!--------------------------- Footer -->
	<footer>
	<div class="container">
		<div class="row text-center">
			<div class="col-lg-4 col-md-12 col-sm-12 col-12">
				<p><a href="index.php" style="color: black;">About JobsForYou</a></p>
				<p><a href="" style="color: black;">hsushwesinoo2002@gmail.com</a></p>
			</div>

			<div class="col-lg-4 col-md-12 col-sm-12 col-12">
				<p><a href="register.php" style="color: black;">Create Account</a></p>
				<p><a href="login.php" style="color: black;">Login Account</a></p>
			</div>

			<div class="col-lg-4 col-md-12 col-sm-12 col-12">
				<p><a href="#" style="color: black;">Facebook</a></p>

				<p><a href="#" style="color: black;">Twitter</a></p>

				<p><a href="#" style="color: black;">Instagram</a></p>
			</div>
		</div>
		<h5 class="text-center">JobsForYou2021. All rights reserved.</h5><hr>
		<p class="text-center">Developed by Hsu Shwe Sin Oo</p>
	</div>
	</footer>


</body>
</html>